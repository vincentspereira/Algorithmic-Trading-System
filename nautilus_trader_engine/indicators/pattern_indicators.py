...
class ConsolidatedPatternDetector(AugmentedIndicator):
    ...
    def __init__(self, config: IndicatorConfig, volume_lookback: int = 20, name: str = "PatternDetector"):
        ...
        self.detected_patterns = deque(maxlen=config.max_memory_items if config.hft_mode else 100)
    ...
    def calculate(self, price: float, volume: float, timestamp: datetime, 
        ...
        if len(self.prices) < 3:  # Need at least 3 candles for most patterns
            return None
        
        # Create candle properties
        ...
        signal_type = self._pattern_to_signal(best_pattern)
