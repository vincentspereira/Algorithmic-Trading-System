...
class VWAP(VolumeWeightedIndicator):
    ...
    def __init__(self, config: IndicatorConfig, reset_period: str = "daily", name: str = "VWAP"):
        ...
        self.vwap_values = deque(maxlen=config.max_memory_items if config.hft_mode else None)
...
class OBV(VolumeWeightedIndicator):
    ...
    def __init__(self, config: IndicatorConfig, name: str = "OBV"):
        ...
        self.obv_value = 0.0
        self.obv_values = deque(maxlen=config.max_memory_items if config.hft_mode else None)
...
class MFI(VolumeWeightedIndicator):
    ...
    def __init__(self, config: IndicatorConfig, name: str = "MFI"):
        ...
        self.typical_prices = deque(maxlen=config.max_memory_items if config.hft_mode else None)
        self.money_flows = deque(maxlen=config.max_memory_items if config.hft_mode else None)
        self.positive_flows = deque(maxlen=config.max_memory_items if config.hft_mode else None)
        self.negative_flows = deque(maxlen=config.max_memory_items if config.hft_mode else None)
    ...
    def calculate(self, price: float, volume: float, timestamp: datetime) -> Optional[IndicatorResult]:
        ...
        # Calculate typical price (using price for all OHLC in this simplified version)
        typical_price = price  # In real implementation: (high + low + close) / 3
        ...
        # Generate signal
        signal, confidence = self._generate_mfi_signal(mfi_value)
...
class AccumulationDistribution(VolumeWeightedIndicator):
    ...
    def __init__(self, config: IndicatorConfig, name: str = "AccumulationDistribution"):
        ...
        self.ad_value = 0.0
        self.ad_values = deque(maxlen=config.max_memory_items if config.hft_mode else None)
    ...
    def calculate(self, price: float, volume: float, timestamp: datetime) -> Optional[IndicatorResult]:
        ...
        # For A/D calculation, we need high, low, close
        # Using price for all in this simplified version
        high = price
        low = price
        close = price
