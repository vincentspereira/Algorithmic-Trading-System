...
@dataclass
class Candle:
    """Wrapper for nautilus_trader Bar to provide candle properties"""
    bar: Bar
    ...
    def is_doji(self) -> bool:
        ...
        return self.body_size / self.total_range <= self.doji_threshold
...
class CandlestickPatternDetector:
    ...
    def add_bar(self, bar: Bar) -> List[PatternResult]:
        """Add a new bar and detect patterns"""
        candle = Candle(bar=bar, doji_threshold=self.doji_threshold)
    ...
    def _determine_trend(self) -> TrendContext:
        ...
        trend_strength = abs(slope * len(closes)) / avg_price if avg_price > 0 else 0
    ...
    def _calculate_volume_confirmation(self, candles: List[Candle]) -> bool:
    ...
    def _assess_pattern_quality(self, candles: List[Candle]) -> float:
        ...
        if len(volumes) > 1:
            volume_consistency = 1.0 - (np.std(volumes) / np.mean(volumes)) if np.mean(volumes) > 0 else 1.0
            quality_score += volume_consistency * 0.1
        
        return np.clip(quality_score, 0.0, 1.0)
    
    # Pattern recognition helper methods
    def _is_hammer_like(self, candle: Candle) -> bool:
    ...
    def _is_shooting_star_like(self, candle: Candle) -> bool:
    ...
    def _is_marubozu(self, candle: Candle) -> bool:
    ...
    def _is_bullish_harami(self, prev: Candle, current: Candle) -> bool:
        """Check for bullish harami pattern"""
        return (prev.is_bearish and current.is_bullish and
                current.open > prev.close and current.close < prev.open)
    
    def _is_bearish_harami(self, prev: Candle, current: Candle) -> bool:
        """Check for bearish harami pattern"""
        return (prev.is_bullish and current.is_bearish and
                current.open < prev.close and current.close > prev.open)
    
    def _is_tweezer_top(self, prev: Candle, current: Candle) -> bool:
        """Check for tweezer top pattern"""
        high_diff = abs(prev.high - current.high) / max(prev.high, current.high) if max(prev.high, current.high) > 0 else 0
        return high_diff <= 0.01 and prev.is_bullish and current.is_bearish
    
    def _is_tweezer_bottom(self, prev: Candle, current: Candle) -> bool:
        """Check for tweezer bottom pattern"""
        low_diff = abs(prev.low - current.low) / max(prev.low, current.low) if max(prev.low, current.low) > 0 else 0
