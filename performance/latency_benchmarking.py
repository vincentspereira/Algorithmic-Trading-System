#!/usr/bin/env python3
"""
Nautilus Trader - Microsecond-Precision Latency Benchmarking
...
class LatencyBenchmark:
    ...
    async def _benchmark_database_operations(self) -> Dict[str, Any]:
        ...
        for operation in operations:
            ...
            measurements = []
            for _ in range(1000):  # Reduced for efficiency
  ...
  def _calculate_latency_statistics(self, operation: str, 
    ...
    def generate_latency_charts(self, results: Dict[str, Any]):
            ...
            for category, benchmarks in results.get("benchmarks", {}).items():
                for operation, stats in benchmarks.items():
                    if isinstance(stats, dict) and "mean_ns" in stats:
                        ...
                        mean_latencies.append(stats["mean_ns"] / 1000.0)  # Convert to microseconds
                        p99_latencies.append(stats["p99_ns"] / 1000.0)
            
            if operations:
                # Mean latency comparison
                axes[0, 0].bar(range(len(operations)), mean_latencies)
                axes[0, 0].set_title("Mean Latency by Operation")
                ...
                axes[0, 1].bar(range(len(operations)), p99_latencies)
                axes[0, 1].set_title("P99 Latency by Operation")
                ...
                axes[1, 0].hist(mean_latencies, bins=20, alpha=0.7, label='Mean')
                axes[1, 0].hist(p99_latencies, bins=20, alpha=0.7, label='P99')
                axes[1, 0].set_title("Latency Distribution")
                axes[1, 0].set_xlabel("Latency (μs)")
                axes[1, 0].set_ylabel("Frequency")
                axes[1, 0].legend()
                
                # Performance classification pie chart
                classifications = {"excellent": 0, "good": 0, "acceptable": 0, "poor": 0}
                
                for latency in mean_latencies:
                    if latency < 100:
                        ...
                        classifications["poor"] += 1
                
                axes[1, 1].pie(classifications.values(), labels=classifications.keys(), autopct='%1.1f%%')
                axes[1, 1].set_title("Performance Classification Distribution")
            
            plt.tight_layout()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plt.savefig(f"latency_benchmark_charts_{timestamp}.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            logger.info(f"Latency benchmark charts saved to latency_benchmark_charts_{timestamp}.png")
            
        except Exception as e:
            logger.error(f"Failed to generate latency charts: {e}")
