...
Submits a new trade order to the Order Management System (OMS), which performs pre-trade risk and compliance checks before routing to a broker.

**Request Body:**

| Field         | Type     | Required | Description                                                     |
|:------------- |:-------- |:-------- |:--------------------------------------------------------------- |
| `account_id`  | `string` | Yes      | UUID of the trading account.                                    |
| `symbol`      | `string` | Yes      | Trading symbol (e.g., `AAPL`, `EUR/USD`, `BTC/USD`).            |
| `side`        | `enum`   | Yes      | `BUY` or `SELL`.                                                |
| `type`        | `enum`   | Yes      | `MARKET`, `LIMIT`, `STOP`, `STOP_LIMIT`, `VWAP`, `TWAP`, `ICEBERG`. |
| `quantity`    | `number` | Yes      | Number of units to trade (e.g., 100 shares).                    |
| `limit_price` | `number` | No       | Price for `LIMIT` or `STOP_LIMIT` orders.                      |
| `stop_price`  | `number` | No       | Stop price for `STOP` or `STOP_LIMIT` orders.                  |
| `strategy_id` | `string` | No       | UUID of the associated strategy (optional for manual orders).   |
...
```json
{
  ...
  "updated_at": "2025-08-21T03:12:05Z"
}
```

#### `PUT /v1/orders/{order_id}`

Modifies an existing order (e.g., updates quantity or price).
...
| `limit_price` | `number` | No       | Updated limit price for `LIMIT` or `STOP_LIMIT` orders.         |
| `stop_price`  | `number` | No       | Updated stop price for `STOP` or `STOP_LIMIT` orders.           |
