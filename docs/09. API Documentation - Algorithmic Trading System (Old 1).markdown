...
### 3.2 Get User by ID
**Description**: Retrieves a user's profile by their ID (admin only).
**HTTP Method**: GET
**Path**: `/users/{user_id}`
...
**Description**: Retrieves a user's profile by their username (admin only).
**HTTP Method**: GET
...
**Example**:
```bash
curl -X GET '/users/username/johndoe' \
-H 'Authorization: Bearer <token>'
```

### 3.4 List All Users
**Description**: Retrieves a paginated list of all registered users (admin only).
**HTTP Method**: GET
**Path**: `/users/`
**Authentication**: Required (Admin role)
...
**Example**:
```bash
curl -X GET '/users/?skip=0&limit=10' \
-H 'Authorization: Bearer <token>'
```

---

## 4. Core Trading Engine API
...
### 6.1 Submit Natural Language Query
...
## 9. Security and Compliance API

### 9.1 Retrieve Audit Logs
