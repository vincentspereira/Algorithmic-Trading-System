...
class BollingerBands(MultiValueIndicator, VolumeConfirmationMixin, SmartMoneyMixin):
    ...
    def __init__(self, config: IndicatorConfig, std_dev: float = 2.0, name: str = "BollingerBands"):
        ...
        self.band_values = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
...
class ATR(VolumeWeightedIndicator, SmartMoneyMixin):
    ...
    def __init__(self, config: IndicatorConfig, name: str = "ATR"):
        ...
        self.true_ranges = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
        self.atr_values = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
        self.atr_value = None
        ...
        self.highs = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
        self.lows = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
        self.closes = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
...
class StandardDeviation(VolumeWeightedIndicator, SmartMoneyMixin):
    ...
    def __init__(self, config: IndicatorConfig, name: str = "StandardDeviation"):
        ...
        self.std_values = deque(maxlen=config.memory_limit if config.enable_hft_optimizations else None)
