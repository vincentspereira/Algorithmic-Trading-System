...
const DashboardPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'trading' | 'health'>('trading');
        ...
        <div className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-4">
          <div className="flex space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg ${activeTab === 'trading' ? 'bg-blue-600' : 'bg-gray-700'}`}
            >
              Trading
            </button>
            <button
              onClick={() => setActiveTab('health')}
              className={`px-4 py-2 rounded-lg ${activeTab === 'health' ? 'bg-blue-600' : 'bg-gray-700'}`}
            >
              Health Dashboard
            </button>
          </div>
